...
class Temp_net:
	...
	def __init__(self,dataset):
		...
		self.Compute_object = {}
		'''
		{'edge':{'train':self.Edge_event_train,'time_weight':self.Edge_dic_weight}}
	...
	def Get_TN(self,agg):
		...
		for t in range(new_nb):
			for k in range(agg):
				...
				for n in range(n1,n2):
					self.TN[t].add_edge(*self.data[n,1:],num=self.Edge_to_ind(*self.data[n,1:]))
		...
		if new_nb*agg<nb_time:
			...
			for k in range(nb_time-agg*new_nb):
				...
				for n in range(n1,n2):
					self.TN[new_nb].add_edge(*self.data[n,1:],num=self.Edge_to_ind(*self.data[n,1:]))
	...
	def Compute_EdgeTNS1(self,t,ind,depth):
		...
		for tau in range(depth):
			...
			if self.TN[t+tau].has_edge(i,j):
				for u in set(self.TN[t+tau][i]).intersection(set(self.TN[t+tau][j])):
					dic_s[u][tau] = '3'
	...
	#compute the sequence of the motif starting at time t of central edge ind
	def Compute_EdgeTNS2(self,t,ind,depth):
		#i,j = self.Ind_to_edge[ind]
		i,j = ind
		#encode the activity profiles of the satellites: dic_s[u] = activity profile of satellite u
		dic_s_i = {}; dic_s_j = {}; dic_s = {}
		for tau in range(depth):
			...
			for u in ngh_j:
				...
				if u in dic_s:
					...
					dic_s[u] = ['0']*depth
					dic_s[u][tau] = '2'
			for u in ngh_i.intersection(ngh_i):
				dic_s[u][tau] = '3'
		central_profile = ''.join(dic_s_i[j])
		del dic_s_i[j]; del dic_s_j[i]; del dic_s[i]; del dic_s[j]
		seq_i = ''.join(sorted([''.join(val) for val in dic_s_i.values()]))
	...
	def Compute_dic_sETN(self,t,v,depth):
		dic_s = {}
		for tau in range(depth):
			#if v is active (i.e. has at least one satellite)
			if v in self.TN[t+tau]:
	...
	#compute the sequence of the motif starting at time t of central edge ind
	def Compute_EdgeTNS_new(self,i,j,depth,dic_s_i,dic_s_j):
		dic_s = {}
		central_profile = dic_s_i[j]
		del dic_s_i[j]; del dic_s_j[i]
		seq_i = ''.join(sorted(dic_s_i.values()))
		seq_j = ''.join(sorted(dic_s_j.values()))
		for u in set(dic_s_i.keys()).union(dic_s_j.keys()):
			if not u in dic_s_i:
				dic_s_i[u] = '0'*depth
			elif not u in dic_s_j:
				dic_s_j[u] = '0'*depth
		if seq_i<=seq_j:
			for u in dic_s_i:
				dic_s[u] = ['0']*depth
				for tau in range(depth):
					dic_s[u][tau] = str(int(dic_s_i[u][tau])+2*int(dic_s_j[u][tau]))
		else:
			for u in dic_s_i:
				dic_s[u] = ['0']*depth
				for tau in range(depth):
					dic_s[u][tau] = str(int(dic_s_j[u][tau])+2*int(dic_s_i[u][tau]))
		return central_profile+''.join(sorted([''.join(val) for val in dic_s.values()]))

	def Add_EdgeTN_new(self,histo,central_nodes,central_edges,t,depth):
		#first compute the NCTN for every central node
		dic_NCTN = {v:self.Compute_dic_sETN(t,v,depth) for v in central_nodes}
		#second use it to compute faster the ECTN
		for ind in central_edges:
			i,j = self.Ind_to_edge[ind]
			dic_s_i = {u:val for u,val in dic_NCTN[i].items()}
			dic_s_j = {u:val for u,val in dic_NCTN[j].items()}
			seq = self.Compute_EdgeTNS_new(i,j,depth,dic_s_i,dic_s_j)
			if seq in histo:
				histo[seq] += 1
			else:
				histo[seq] = 1
	...
	def Add_EdgeTN(self,histo,central_edges,t,depth,choice=0):
		if choice==0:
			func = self.Compute_EdgeTNS
		elif choice==1:
			func = self.Compute_EdgeTNS1
		elif choice==2:
			func = self.Compute_EdgeTNS2
		for ind in central_edges:
			seq = func(t,ind,depth)
			if seq in histo:
				histo[seq] += 1
			else:
				histo[seq] = 1
	...
	def EdgeTN_histo_old(self,depth):
	...
	def EdgeTN_histo1(self,depth):
		...
		for t in range(len(self.TN)-depth+1):
			...
			#update the EdgeTN histogram
			histo = self.Add_EdgeTN(histo,central_edges,t,depth)
		return histo

	def EdgeTN_histo2(self,depth):
		#histo[seq] = nb of occurrences of EdgeTN seq in the whole temporal network
		nb_time = len(self.TN)
		list_times = range(nb_time-depth+1)
		#cumulate the interactions on depth time steps to keep track of the central edges
		...
		for tau in range(depth):
			for edge in self.TN[tau].edges:
				...
				if ind in dic_central_edges:
					...
					dic_central_edges[ind] = 1
		histo = self.Add_EdgeTN({},dic_central_edges.keys(),0,depth,choice=1)
		for t in list_times[1:]:
			#remove the edges from time t-1
			for edge in self.TN[t-1].edges:
				ind = self.Edge_to_ind(*edge)
			...
			for ind in edges_to_remove:
				del dic_central_edges[ind]
			#update the EdgeTN histogram
			histo = self.Add_EdgeTN(histo,dic_central_edges.keys(),t,depth,choice=1)
		return histo

	def EdgeTN_histo3(self,depth):
		#histo[seq] = nb of occurrences of EdgeTN seq in the whole temporal network
		...
		for tau in range(depth):
			for edge in self.TN[tau].edges:
				...
				if ind in dic_central_edges:
					...
					dic_central_edges[ind] = 1
		histo = self.Add_EdgeTN({},dic_central_edges.keys(),0,depth,choice=2)
		for t in list_times[1:]:
			#remove the edges from time t-1
			for edge in self.TN[t-1].edges:
				if edge[0]<edge[1]:
			...
			for ind in edges_to_remove:
				del dic_central_edges[ind]
			#update the EdgeTN histogram
			histo = self.Add_EdgeTN(histo,dic_central_edges.keys(),t,depth,choice=2)
		return histo

	def EdgeTN_histo3_old(self,depth):
		#histo[seq] = nb of occurrences of EdgeTN seq in the whole temporal network
		list_times = range(len(self.TN)-depth+1)
		#cumulate the interactions on depth time steps to keep track of the central edges
		dic_central_edges = {}
	...
	def EdgeTN_histo4(self,depth):
		#histo[seq] = nb of occurrences of EdgeTN seq in the whole temporal network
		nb_time = len(self.TN)
		list_times = range(nb_time-depth+1)
		#cumulate the interactions on depth time steps to keep track of the central edges
		dic_central_edges = {}; central_nodes = set(())
		for tau in range(depth):
			central_nodes = central_nodes.union(set(self.TN[tau].nodes))
			for edge in self.TN[tau].edges:
				ind = self.Edge_to_ind(*edge)
				if ind in dic_central_edges:
					dic_central_edges[ind] += 1
					...
					dic_central_edges[ind] = 1
		histo = self.Add_EdgeTN_new({},central_nodes,dic_central_edges.keys(),0,depth)
		for t in list_times[1:]:
			central_nodes = set(())
			for tau in range(depth):
			...
			for ind in edges_to_remove:
				del dic_central_edges[ind]
			#update the EdgeTN histogram
			histo = self.Add_EdgeTN_new(histo,central_nodes,dic_central_edges.keys(),t,depth)
		return histo

	def Compute_EdgeTNS_new_net(self,i,j,depth,dic_s_i,dic_s_j):
		dic_s = {}
		central_profile = dic_s_i[j]
		del dic_s_i[j]; del dic_s_j[i]
		add_i = set(()); add_j = set(())
		for u in set(dic_s_j).difference(set(dic_s_i)):
			dic_s_i[u] = '0'*depth
			add_i.add(u)
		for u in set(dic_s_i).difference(set(dic_s_j)):
			dic_s_j[u] = '0'*depth
			add_j.add(u)
		for u in dic_s_i:
			dic_s[u] = ['0']*depth
		...
		for u in add_j:
			del dic_s_j[u]
		dic_s_i[j] = central_profile; dic_s_j[i] = central_profile
		return central_profile+''.join(sorted([''.join(val) for val in dic_s.values()]))

	def Add_EdgeTN_new_net(self,histo,central_net,t,depth):
		for i in central_net:
			dic_s_i = self.Compute_dic_sETN(t,i,depth)
			seq_i = ''.join(sorted(dic_s_i.values()))
			for j in central_net[i]:
				dic_s_j = self.Compute_dic_sETN(t,j,depth)
				seq_j = ''.join(sorted(dic_s_j.values()))
				if seq_i<=seq_j:
					seq = self.Compute_EdgeTNS_new_net(i,j,depth,dic_s_i,dic_s_j)
				else:
					seq = self.Compute_EdgeTNS_new_net(j,i,depth,dic_s_j,dic_s_i)
				if seq in histo:
					histo[seq] += 1
				else:
					histo[seq] = 1
	...
	def EdgeTN_histo5(self,depth):
		#histo[seq] = nb of occurrences of EdgeTN seq in the whole temporal network
		nb_time = len(self.TN)
		list_times = range(nb_time-depth+1)
		#cumulate the interactions on depth time steps to keep track of the central edges
		central_net = nx.DiGraph()
		for tau in range(depth):
			for edge in self.TN[tau].edges:
				if edge[0]<edge[1]:
					i,j = edge
					...
					i,j = edge[::-1]
				if central_net.has_edge(i,j):
					central_net[i][j]['weight'] += 1
				else:
					central_net.add_edge(i,j,weight=1)
		histo = self.Add_EdgeTN_new_net({},central_net,0,depth)
		for t in list_times[1:]:
			#remove the edges from time t-1
			for edge in self.TN[t-1].edges:
				if edge[0]<edge[1]:
			...
			for edge in self.TN[t+depth-1].edges:
				...
				if central_net.has_edge(i,j):
					...
					central_net.add_edge(i,j,weight=1)
			central_net.remove_edges_from({edge for edge in central_net.edges if central_net[edge[0]][edge[1]]['weight']<=0})
			#update the EdgeTN histogram
			histo = self.Add_EdgeTN_new_net(histo,central_net,t,depth)
		return histo
	...
	def ETN_histo_old_new(self,depth):
		histo = {}
		for t in range(len(self.TN)-depth+1):
			dic_s = nx.DiGraph()
			for tau in range(depth):
				for i,j in self.TN[t+tau].edges:
					for u,v in zip([i,j],[j,i]):
						if not v in dic_s:
							weight = ['0']*depth; weight[tau] = '1'
							dic_s.add_edge(v,u,weight=weight)
						else:
							if u in dic_s[v]:
								dic_s[v][u]['weight'][tau] = '1'
							else:
								weight = ['0']*depth; weight[tau] = '1'
								dic_s.add_edge(v,u,weight=weight)
			for v in dic_s.nodes:
				seq = ''.join(sorted([''.join(dic_s[v][u]['weight']) for u in dic_s[v]]))
				if seq in histo:
					histo[seq] += 1
				else:
					histo[seq] = 1
	...
	#compute dic_ETN using a multigraph
	def ETN_histo_new(self,depth):
		histo = {}
		cum_graph = nx.MultiGraph()
		#cumulate the interactions on depth consecutive time steps
		for tau in range(depth):
			for edge in self.TN[tau].edges:
				cum_graph.add_edge(*edge,key=tau)
		for central_node in cum_graph:
			dic_s = {u:['0']*depth for u in cum_graph.neighbors(central_node)}
			for edge in cum_graph.edges(central_node,keys=True):
				dic_s[edge[1]][edge[2]] = '1'
			seq = ''.join(sorted([''.join(val) for val in dic_s.values()]))
			if seq in histo:
				histo[seq] += 1
			else:
				histo[seq] = 1
		...
		for t in range(1,len(self.TN)-depth+1):
			#remove the interactions at t-1
			for edge in self.TN[t-1].edges:
				cum_graph.remove_edge(*edge,key=t-1)
			#add the interactions at t+depth-1
			for edge in self.TN[t+depth-1].edges:
				cum_graph.add_edge(*edge,key=t+depth-1)
			#remove the nodes of degree zero
			cum_graph.remove_nodes_from({i for i in self.TN[t-1].nodes if cum_graph.degree(i)==0})
			#collect the new motifs
			for central_node in cum_graph:
				dic_s = {u:['0']*depth for u in cum_graph.neighbors(central_node)}
				for edge in cum_graph.edges(central_node,keys=True):
					dic_s[edge[1]][edge[2]-t] = '1'
				seq = ''.join(sorted([''.join(val) for val in dic_s.values()]))
				if seq in histo:
					histo[seq] += 1
				else:
	...
	def ETN_histo(self,depth):
		central_nodes = [0]*self.info['N']
		set_active = set(())
		for tau in range(depth):
			for i in self.TN[tau].nodes:
				central_nodes[i] += 1
				set_active.add(i)
		histo = self.Add_ETN({},set_active,0,depth)
		list_set_nodes = [set(val.nodes) for val in self.TN.values()]
		for t in range(1,len(self.TN)-depth+1):
			set_doubt = list_set_nodes[t-1].difference(list_set_nodes[t+depth-1])
			set_growth = list_set_nodes[t+depth-1].difference(list_set_nodes[t-1])
			for i in set_doubt:
				central_nodes[i] -= 1
				if central_nodes[i]<1:
					set_active.remove(i)
			for i in set_growth:
				central_nodes[i] += 1
				set_active.add(i)
			histo = self.Add_ETN(histo,set_active,t,depth)
		return histo
