...
from libs.settings import XP_data,Setup_Plot,Vector_distance,Cosim,Get_versions,Load_instance_param,Get_savename,Raw_to_binned,Load_TN_ADM
from libs.atn import ADM_class,Min_EW,Life_game
import libs.ETN as etn_lib
...
def Scalar_from_distr(name,obs,list_b,list_agg):
	...
	obsarg_to_num = [Find_folder(*obs) for obs in obs_with_arg]
	for agg in list_agg:
...
def Display_symbol(string,tab,title=''):
...
#use cross validation instead of MDL to determine the best degree for the fitting polynom
def Get_optipol_fit_V2(Y,pol_mat):
	n = len(Y); X = np.linspace(0,1,n)
	#normalise Y
	m,M = min(Y),max(Y)
	Y = (Y-m)/(M-m)
	dic_errors = {deg:Cross_valopti(Y,pol_mat[deg]) for deg in range(min(len(Y),11))}
	best_deg = min(dic_errors.keys(),key=lambda deg:dic_errors[deg])
	return np.polyfit(X,Y,best_deg)

def Cross_val(deg,Y):
	n = len(Y); errors = []; X = np.linspace(0,1,n)
	for pos in range(n):
...
def Cross_valopti(Y,list_mat):
	n = len(Y); errors = []; X = np.linspace(0,1,n)
	for pos in range(n):
		YY = np.concatenate((Y[:pos],Y[pos+1:]))
		pol_fit = np.dot(list_mat[pos][1],np.dot(list_mat[pos][0].T,YY))
		errors.append((np.polyval(pol_fit[::-1],X[pos])-Y[pos])**2)
	return np.mean(errors)
...
def Get_optistring_V5(Y,pol_mat):
	sign_der = np.sign(Y[1:]-Y[:-1])
	if (sign_der<0).all():
		return '-'*len(sign_der)
	elif (sign_der>0).all():
	...
	der_tab = Y[1:]-Y[:-1]
	#interpolate the curve with a polynom defined on [0,1] using MDL and consider two possibilities
	pol_fit = Get_optipol_fit_V2(Y,pol_mat)
	#if the polynom is constant then the string is full of zeros
	if len(pol_fit)==1:
		return '0'*len(sign_der)
...
def Get_dic_mat(X,deg_max):
	M = np.ones((len(X),deg_max+1))
	for j in range(1,deg_max+1):
		M[:,j] = M[:,j-1]*X
	return {deg:(M[:,:deg+1],np.linalg.inv(np.dot(M[:,:deg+1].T,M[:,:deg+1]))) for deg in range(deg_max+1)}
...
Get_list_TN(list_obs,'myarchive6'); exit()
'''
'''
for name in ['conf16','utah','brownD1']:
	temp_net = tp.Temp_net(tp.Load_TN(name))
	temp_net.Get_data_time()
	print(name,len(temp_net.data_time))
'''
pass
#save data under binary format peckle
#try to estimate the level of confidence of the algo (fourth letter)
#Sphinx python: convert comments into latex or html documentation
pass
class Data_to_symb:
	"""
	"""
	...
	#load dic_b_sentence
	def Load_dic_b_sentence(self,average=True):
		if average:
			savepath = 'codata/nb_flow/noise_averaging/'
		else:
			savepath = 'codata/nb_flow/default/'
		standard_obs = Rewrite_listobs(self.int_scalar_obs)
		int_to_obs = {i:str(obs) for i,obs in enumerate(standard_obs)}
		tab = np.loadtxt(savepath+'int_to_name.txt',dtype=str)
		int_to_name = {int(tab[0,i]):tab[1,i] for i in range(np.size(tab,1))}
		tab = np.loadtxt(savepath+'dic_b_sentence.txt',dtype=str)
		dic_b_sentence = {}
		#identify the longest sentence
		longest_sentence = ''; longest_obs = ''; longest_name = ''
		for i,obs in int_to_obs.items():
			dic_b_sentence[obs] = {}
			for j,name in int_to_name.items():
				if len(tab[i,j])>len(longest_sentence):
					longest_sentence = tab[i,j]
					longest_obs = obs; longest_name = name
				dic_b_sentence[obs][name] = tab[i,j]
		'''
		...
		'''
		#display the associated curve
		#Display_symbol(string,tab)
		return dic_b_sentence

	#load human evaluation of the sign sequences for different observables and datasets
	def Load_human(self):
		human_sign = {}
		with open('codata/nb_flow/human_sign.txt','r') as file:
			line = ''; int_to_name = {}
			#first collect the correspondance btw integer identifier and dataset name
			#this correspondance is valid only in the file
			while True:
				line = file.readline()
				if line=='\n':
					break
				...
				while line[ind]!=' ':
					ind += 1
				int_to_name[int(line[:ind])] = line[ind+1:-1]
			#collect the sign sequences
			while True:
				obs = file.readline()[:-1]
				human_sign[obs] = {}
				for i,name in int_to_name.items():
					human_sign[obs][name] = file.readline()[:-1]
				if not file.readline():
					break
			return human_sign,list(int_to_name.values()),list(human_sign.keys())

	#compare the human with algo outcome
	def Evaluate_get_str(self,average=True):
		human_sign,list_savename,list_obs = self.Load_human()
		algo_sign = self.Load_dic_b_sentence(average=average)
		nb_wrong = 0
		for obs in list_obs:
			for name in list_savename:
				nb_wrong += int(algo_sign[obs][name]!=human_sign[obs][name])
		error_rate = nb_wrong*100/(len(list_obs)*len(list_savename))
	...
	#for all levels of agg, display the flow vs b of obs for name as well as the symbolic interpretation
	def Display_vs_b(self,obs,name):
		if obs in Rewrite_listobs(self.ext_obs):
			self.Load_b_flow = self.Load_ext_b_flow
		else:
			self.Load_b_flow = self.Load_scalar_b_flow
		self.obs_to_folder = {obs:str(Find_folder(*obs))}
		self.Get_dic_pol_mat()
		for agg in self.tot_agg:
			print(str(agg)+' begins')
			tab = np.array([self.list_b,self.Load_b_flow(obs,name,agg)])
			string = self.Get_str(obs,name,agg)
			#string = '0'*(len(self.list_b)-1)
			Display_symbol(string,tab,title=r'$n = $'+str(agg))
		plt.show()
		exit()

	#display everything for one given level of agg, obs and name
	def Investigate_single(self,obs,name,agg):
		self.Load_b_flow = self.Load_scalar_b_flow
		self.obs_to_folder = {obs:str(Find_folder(*obs))}
		self.Get_dic_pol_mat()
		tab = np.array([self.list_b,self.Load_b_flow(obs,name,agg)])
		string = self.Get_str(obs,name,agg,display=True)
		Display_symbol(string,tab,title=r'$n = $'+str(agg))
		plt.show()
		exit()

	#display XP curves either for every agg (agg=None) or the specified agg
	#either the curves are averaged over ten noise (local TS) realizations
	#obs and name are already under standard form
	def Visu_XP(self,obs,name,agg=None,archive_choice='sliding',savefig=False,title=''):
		if archive_choice=='sliding':
			self.archive = zipfile.ZipFile('myarchive_sliding.zip','r')
			if savefig:
				title = ' (sliding TS)'; savefig = '_STS'
		elif archive_choice=='discrete':
			if savefig:
				title = ' (discrete TS)'; savefig = '_DTS'
			self.archive = zipfile.ZipFile('myarchive_average_all_XP.zip','r')
		elif archive_choice=='raha':
			if savefig:
				title = ' (discrete TS)'; savefig = '_DTS'
			self.archive = zipfile.ZipFile('myarchive_average_raha.zip','r')
		else:
			self.archive = zipfile.ZipFile(archive_choice+'.zip','r')
			if savefig:
				title = ' (sliding TS)'; savefig = '_STS'
		if agg is None:
			list_agg = self.tot_agg
		elif type(agg)==list:
			list_agg = agg
		else:
			list_agg = [agg]
		scalar = False; list_real = range(10)
		if '0avg' in obs[0]:
			scalar = True; distr_obs = (obs[0][:-4],obs[1])
			folder_num = str(self.Find_folder_local(*distr_obs))
			func = lambda tab:np.sum(tab[0,:]*tab[1,:])/np.sum(tab[1,:])
		elif '0frac' in obs[0]:
			scalar = True; distr_obs = (obs[0][:-5],obs[1])
			folder_num = str(self.Find_folder_local(*distr_obs))
			func = lambda tab:np.sum(tab[0,:]**2*tab[1,:])/np.sum(tab[0,:]*tab[1,:])
		else:
			folder_num = str(self.Find_folder_local(*obs))
		if not scalar:
			if not '0sim' in obs[0]:
				Load_b_flow = lambda n,surname:self.Load_scalar_new(obs,name,n,folder_num,surname=surname)
			else:
				Load_b_flow = lambda n:self.Load_etn_sim(obs,name,n,folder_num)
				list_real = None
		else:
			Load_b_flow = lambda n,surname:self.Load_scalarized(distr_obs,name,n,folder_num,func,surname=surname)
		#load XP data
		'''
		dic_tab = {b:{} for b in self.list_b}
		for n in list_agg:
			tab = Load_b_flow(n)
			for i,b in enumerate(self.list_b):
				dic_tab[b][n] = tab[i]
		fig,ax = plt.subplots(1,1,constrained_layout=True)
		ax.set_title('word vs n')
		ax.set_facecolor('gray')
		cmap = get_cmap('Spectral')
		b_norm = [(b-self.list_b[0])/(self.list_b[-1]-self.list_b[0]) for b in self.list_b]
		for b,val in zip(self.list_b,b_norm):
			ax.plot(*zip(*dic_tab[b].items()),color=cmap(val))
		plt.show()
		exit()
		'''
		for n in list_agg:
			Y = np.zeros(self.n)
			nb = 0; fontsize = 15; ylabel = ''
			if savefig:
				if obs[1]:
					ylabel = obs[0]+','+str(obs[1][0])
					obspath = obs[0]+str(obs[1][0])
				else:
					ylabel = obs[0]
					obspath = obs[0]
			fig,ax = Setup_Plot('',ylabel,fontsize=fontsize)
			ax.set_title('n = '+str(n)+title,fontsize=fontsize)
			if list_real is not None:
				for i in list_real:
					tab = Load_b_flow(n,'real'+str(i))
					ax.plot(self.list_b,tab,'.',color='red')
					Y += tab
					nb += 1
				ax.plot(self.list_b,Y/nb,'--',color='k')
			else:
				tab = Load_b_flow(n)
				ax.plot(self.list_b,tab,'.',color='r')
				ax.plot(self.list_b,tab,'--',color='k')
			if savefig:
				path = 'figures/new_flows/'+obspath
				if not os.path.isdir(path):
					os.mkdir(path)
				path += '/'+name
				if not os.path.isdir(path):
					os.mkdir(path)
				plt.savefig(path+'/'+str(n)+'agg'+savefig+'.png')
		if not savefig:
			plt.show()
		plt.close('all')

	def Get_list_obs(self):
		self.int_scalar_obs = []#['edge0nb_tot','edge0nb_diff']
		self.int_distr_obs = ['edge0time_weight']
		for obj in ['node','edge']:
			for prefix in ['','event_','inter']:
				self.int_distr_obs.append(obj+'0'+prefix+'duration')
		for motif in ['NCTN','ECTN']:
			for prefix in ['motif_error','nb_diff','nb_tot']:
				for depth in [2,3]:
					self.int_scalar_obs.append((motif+'0'+prefix,depth))
		self.int_scalar_obs += ['ICC','avg_cc']
		self.int_distr_obs += ['inst_deg']#['inst_deg','cc_size']
		#add the scalarized observables
		for obs in self.int_distr_obs:
			self.int_scalar_obs.append(obs+'0avg')
			self.int_scalar_obs.append(obs+'0frac')
		
		#add the external observables
		for depth in [2,3]:
			for el in ['NCTN','ECTN']:
				for suffix in ['','_trunc']:
					self.ext_obs.append((el+'0sim'+suffix,depth))

	def Pol_to_string_localold(self,pol_fit,Y_start):
	...
	def Pol_to_string_local(self,pol_fit,Y_start):
		...
		while keep:
			...
			if (M_remainder-m_remainder)/(M_tot-m_tot)<0.05:
				return ''.join(res[:end+1])+letter*(len(res)-end-1)#self.Check_last_domain(pol_fit,''.join(res[:end+1])+letter*(len(res)-end-1))
			else:
				ind += 1
				if ind==len(domains):
					keep = False
		return ''.join(res)#self.Check_last_domain(pol_fit,''.join(res))

	#test the hypothesis the last domain is a spurious realization of noise, compatible with the domain
	#before it
	#other idea: test the second most frequent string: if the only difference with the most frequent is
	#the last domain, then the simplest string is returned as description
	def Check_last_domain(self,pol_fit,string):
		domains = self.Find_domains_local(string)
		if len(domains)<=1:
			return string
		start = domains[-1][0]
		X = np.linspace(0,1,len(string)+1)
		#prolungate the polynomial fit by a linear function at the start of the last domain
		#y = a*x+b
		der_pol = np.zeros(len(pol_fit)-1)
		for k in range(len(der_pol)):
			der_pol[k] = (len(pol_fit)-k-1)*pol_fit[k]
		pass
		#a and b should be estimated by linear regression on the domain before the last domain
		pass
		a = np.polyval(der_pol,X[start-1])
		b = np.polyval(pol_fit,X[start-1])-a*X[start-1]
		pass
		#see the data points of the last domain as independent Gaussian variables centered around the
		#linear prolungation and of std equal to the fit confidence interval
		list_mu = a*X[start:] + b
		#compute the proba that all data points of the last domain are at a distance to their mean greater
		#than they actually are
		proba = 1
		for mu,std,y in zip(list_mu,self.list_std,self.Y_data[start:]):
			z = abs(y-mu)/(np.sqrt(2)*std)
			proba *= erf(z)
		return string
		print(proba)
		if proba<0.01:
			return string[:start]+string[start-1]*(len(string)-start)
		return string

	def Load_scalar_new(self,obs,savename,agg,folder_num,surname=''):
		path = 'codata/'+obs[0]+'/'+folder_num+'/Flow_'+savename+'_n'+str(agg)+surname+'.txt'
		return np.loadtxt(io.BytesIO(self.archive.read(path)))

	def Load_etn_sim(self,obs,savename,agg,folder_num):
		path = 'codata/'+obs[0]+'/'+folder_num+'/Flow_'+savename+'_n'+str(agg)+'vsb.txt'
		return np.loadtxt(io.BytesIO(self.archive.read(path)))[1,:]

	def Load_scalarized(self,distr_obs,savename,agg,folder_num,func,surname=''):
		res = np.zeros(len(self.list_b))
		path = 'codata/'+distr_obs[0]+'/'+folder_num+'/Flow_'+savename+'_n'+str(agg)+'_b'
		for i,b in enumerate(self.list_b):
	...
	def Load_scalar_b_flow(self,obs,savename,agg,surname=''):
		scalar = False
		if '0avg' in obs[0]:
			scalar = True; distr_obs = (obs[0][:-4],obs[1])
			func = lambda tab:np.sum(tab[0,:]*tab[1,:])/np.sum(tab[1,:])
		...
		if not scalar:
			...
			return np.loadtxt(io.BytesIO(self.archive.read(path)))
		res = np.zeros(len(self.list_b))
		path = 'codata/'+distr_obs[0]+'/'+self.obs_to_folder[distr_obs]+'/Flow_'+savename+'_n'+str(agg)+'_b'
		for i,b in enumerate(self.list_b):
			tab = np.loadtxt(io.BytesIO(self.archive.read(path+str(b)+surname+'.txt')))
			if tab.ndim==1:
				if tab[0]<0.5:
	...
	def Load_ext_b_flow(self,obs,savename,agg,surname=''):
		return np.loadtxt('codata/'+obs[0]+'/'+self.obs_to_folder[obs]+'/Flow_'+savename+'_n'+str(agg)+surname+'vsb.txt')[1,:]
	...
	pass
	#the polynomial fit is much more sensitive to the last point than to any intermediate point:
	#the last point is more prone to overfit than intermediate points
	#then the last domain can be attributed the wrong sign
	#to handle this: either the last point is neutralized by setting Y[-1] = Y[-2] or more generally
	#we move the data points so that the curve fits as most as possible as a polynom:
	#we compute the points on which the prediction error gradient is the highest (the most sensitive points)
	#an optimal curve is such that if we move points a little, the fit does not change very much
	#our gradient contains the derivative of the prediction error wrt each data point at the MLE
	#at fixed degree we find the fit whose derivative of the fit parameters prediction error is zero on
	#the training set but we want zero on the test set. As this cannot be done directly
	#(otherwise the test set becomes just part of the training set), we modify the data points themselves,
	#while the variable wrt we derive has changed. (we want the fit parameters not to change wrt the data)
	pass
	#other idea: replace the polynomial fit by piecwise linear functions so that (1) they fit in the CI
	#of the pol fit and (2) they give rise to the most simple string description
	#generally the difficulty is that the algo does not estimate confidence about its prediction:
	#it should control the hypotheses it makes and deduce the most appropriate string description
	#actually maybe an AI system is more appropriate
	#a human, after analyzing a first curve, takes the last curve as a prior for the next one
	#a human can also handle more complex priors like succession rules, e.g. if often -+ is seen after a -
	#then the priors after a - will be - and -+
	#it is faster to test for a given prior than redoing the whole analysis of a new curve (that is why
	#humans are good at handling priors)
	def Get_str(self,obs,name,agg,display=False,average=True):
		surname = 'real'; Y = np.zeros(self.n)
		for i in range(10):
			Y += self.Load_b_flow(obs,name,agg,surname=surname+str(i))
		#first check for monotonicity
		sign_der = np.sign(Y[1:]-Y[:-1])
		if (sign_der<0).all():
			return '-'*len(sign_der)
		...
		if display:
			fig,ax = plt.subplots(1,1,constrained_layout=True)
			n = len(Y); X = np.linspace(0,1,n); XX = np.linspace(0,1,100)
			ax.plot(X,Y,'.')
			ax.plot(XX,np.polyval(pol_fit,XX))
			ax.plot(X,low_ci.flatten(),color='green'); ax.plot(X,up_ci.flatten(),color='green')
			plt.show()
		#we add Gaussian noise to the data with std equal to half the CI width
		#then we choose as description the most frequent one
		...
		while keep:
			...
			if ind==len(dic_sub):
				keep = False
		second_string = min(sorted_key[max_pos:ind],key=lambda s:len(String_to_letter(s)))
		#compare string and second_string
		letter1 = String_to_letter(string); letter2 = String_to_letter(second_string)
		...
		if display:
			...
			for s in sorted_key[:max_pos]:
				print(s)
			print('second most frequent strings:')
			for s in sorted_key[max_pos:ind]:
				print(s)
			print('chosen string:')
			print(chosen_string)
			print('end of function')
			plt.show()
		return chosen_string#[:(self.n-1)//2]

	#convert 1D flows into strings, assuming that Rewrite_listobs and Get_savename have not been run yet
	#if average==True, average data curves over ten realizations of the local TS to decrease noise and
	#then perform the symbolic conversion
	def Tab_to_str(self,list_name,type_obs,average=True):
		if average:
			suffix = 'noise_averaging'
		else:
			suffix = 'default'
		if type_obs=='int_scalar':
			self.Load_b_flow = self.Load_scalar_b_flow
			standard_obs = Rewrite_listobs(self.int_scalar_obs)
		elif type_obs=='ext':
			self.Load_b_flow = self.Load_ext_b_flow
			standard_obs = Rewrite_listobs(self.ext_obs)
		else:
			raise ValueError('implemented obs type: ext and int_scalar')
		self.archive = zipfile.ZipFile('myarchive.zip','r')
		standard_name = [Get_savename(name) for name in list_name]
		self.obs_to_folder = {}
		for obs in standard_obs:
			if '0avg' in obs[0]:
				distr_obs = (obs[0][:-4],obs[1])
				self.obs_to_folder[distr_obs] = str(Find_folder(*distr_obs))
			elif '0frac' in obs[0]:
		...
		#t_start = time.time(); print('elapsed time:',time.time()-t_start)
		dic_b_sentence = {}
		for obs in standard_obs:
			print(obs)
			dic_b_sentence[obs] = {}
			for name in standard_name:
				letter = ''; sentence = ''
				for agg in self.tot_agg:
					new_letter = String_to_letter(self.Get_str(obs,name,agg,average=average))
					if new_letter!=letter:
						sentence += '|'+new_letter
						letter = new_letter
				dic_b_sentence[obs][name] = sentence[1:]
		#save data
		savepath = 'codata/nb_flow/'
		if not os.path.isdir(savepath):
			os.mkdir(savepath)
		savepath += suffix+'/'
		if not os.path.isdir(savepath):
			os.mkdir(savepath)
		#convert the dictionary into a numpy array by mapping obs and name to integers
		self.int_to_obs = {i:obs for i,obs in enumerate(standard_obs)}
		self.int_to_name = {i:name for i,name in enumerate(standard_name)}
		np.savetxt(savepath+'int_to_name.txt',np.array(list(zip(*self.int_to_name.items())),dtype=str),fmt='%s')
		res = np.array([[dic_b_sentence[self.int_to_obs[i]][self.int_to_name[j]] for j in range(len(standard_name))] for i in range(len(standard_obs))],dtype=str)
		np.savetxt(savepath+'dic_b_sentence.txt',res,fmt='%s')
		save_obs = {}
		for i,obs in enumerate(self.int_scalar_obs):
			if type(obs)==tuple:
				save_obs[i] = obs[0]+'^'+str(obs[1])
			else:
				save_obs[i] = obs
		np.savetxt(savepath+'int_to_obs_int_scalar.txt',np.array(list(zip(*save_obs.items())),dtype=str),fmt='%s')

#compare ranges of STS and LTS
def Comp_TS(choice):
	T = 1000; b = 10
	list_times = list(range(T))
	#apply STS
	if choice=='STS':
	...
	#apply LTS
		...
		for k in range(nb_blocks):
			list_times[k*b:(k+1)*b] = rd.sample(list_times[k*b:(k+1)*b],b)
		if b*nb_blocks<T:
			k = nb_blocks
			list_times[k*b:] = rd.sample(list_times[k*b:],T-b*nb_blocks)
	#compute distances travelled by timestamps
	travel_distances = []
	for ind,t in enumerate(list_times):
		travel_distances.append(ind-t)
	#print the average travelled distance
	print('avg trav dist under '+choice+': '+str(np.std(travel_distances)))
	#shows the histogram
	fig,ax = plt.subplots(constrained_layout=True)
	ax.hist(travel_distances)
	plt.show()
#Comp_TS('LTS'); exit()

list_name = [('ADM',9,'conf16'),('ADM',18,'conf16'),'conf16',('min_ADM',1),('min_ADM',2),('min_EW',1),('min_EW',2),('min_EW',3),'utah']
data_to_symb = Data_to_symb()
#data sets missing in archive_sliding.zip: 'min_ADM2', 'utah'
#data_to_symb.Get_list_obs()
#('ECTN0motif_error',(3,)) ('edge0event_duration0avg',()) ('ICC', ()) ('NCTN0nb_diff', (2,)) ('edge0interduration0frac', ())
obs = ('ECTN0motif_error',(3,)); name = 'ABPpi4'
data_to_symb.Visu_XP(obs,name,agg=None,archive_choice='arch10',savefig=False); exit()

#('NCTN0nb_diff', (3,)) is too noisy with DTS but just fine with STS !!!
list_name = [('ADM',9,'conf16'),('ADM',18,'conf16'),'conf16',('min_ADM',1),('min_EW',2),('min_EW',3)]
for obs in [('NCTN0nb_diff', (2,)),('NCTN0motif_error', (2,)),('ECTN0nb_diff', (2,)),('ECTN0nb_diff', (3,)),('ECTN0motif_error', (3,)),('ICC', ()),('edge0event_duration0avg', ()),('edge0duration0avg', ())]:
	print(obs)
	for name in list_name:
		name = Get_savename(name)
		print(name)
		data_to_symb.Visu_XP(obs,name,agg=None,archive_choice='sliding',savefig=True)
exit()
data_to_symb.Tab_to_str(list_name,'int_scalar',average=True); exit()
data_to_symb.Evaluate_get_str(average=True); exit()
...
list_obs = ['edge0time_weight']
for obj in ['node','edge']:
	for prefix in ['','event_','inter']:
		list_obs.append(obj+'0'+prefix+'duration')
list_obs += ['inst_deg','cc_size']
for name in list_name:
	print(name)
	for obs in list_obs:
		print('\t'+obs)
		Scalar_from_distr(name,Rewrite_obs(obs),list_b,list_agg)
exit()
'''
